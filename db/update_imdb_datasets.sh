...
import_to_sqlite() {
    ...
    sqlite3 imdb_temp.db "PRAGMA journal_mode = OFF;"
    echo "Beginning transaction..."
    sqlite3 imdb_temp.db "BEGIN TRANSACTION;"

    for dataset in "${datasets[@]}"; do
        table_name="${dataset//./_}"
        echo "Importing $dataset.tsv into $table_name..."
        sqlite3 imdb_temp.db ".mode tabs" ".import $dataset.tsv $table_name"
    done

    echo "Committing transaction..."
    sqlite3 imdb_temp.db "COMMIT;"
    end_time=$(date +%s)
    duration=$(( end_time - start_time ))
    echo "Data import into SQLite database completed in $duration seconds"

    cd ..
    echo "All datasets have been downloaded, extracted, preprocessed, and imported into imdb_temp.db."
}

# Replace old database with new one
replace_database() {
    echo "Replacing old database with new database..."
    mv imdb_temp.db imdb.db
